...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const modernRes = await fetch(`${import.meta.env.VITE_API_URL || ''}/api/auth/login`, {
        method: 'POST',
    ...
    <div className="min-h-screen relative flex items-center justify-center px-4 py-8">
      ...
      <div className="relative z-10 w-full max-w-md">
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <div>
            <p className="text-white text-sm tracking-widest uppercase opacity-90">Système de Gestion</p>
            <p className="text-blue-100 text-xs">École Primaire</p>
          </div>
        </div>
