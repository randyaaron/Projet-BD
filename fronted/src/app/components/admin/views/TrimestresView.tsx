...
import { CalendarDays, Plus, Search, Edit2, Trash2, CheckCircle2, XCircle, X } from 'lucide-react';
...
export function TrimestresView() {
  ...
  const [terms, setTerms] = useState<Trimestre[]>([]);
  const [loading, setLoading] = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [saving, setSaving] = useState(false);
  const [newTerm, setNewTerm] = useState({ libelle: '' });

  const fetchTerms = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/legacy/trimestres`);
      if (res.ok) {
  ...
  const handleCreate = async () => {
    if (!newTerm.libelle) return;
    try {
      setSaving(true);
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/legacy/trimestres`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ libelle: newTerm.libelle })
      });
      if (res.ok) {
        setShowModal(false);
        setNewTerm({ libelle: '' });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Trimestres</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Gestion des périodes d'évaluation pour {schoolName}</p>
        </div>
        <button 
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-8 text-center text-slate-500">Aucun trimestre</td>
                </tr>
              ) : terms.map((term) => (
                <tr key={term.idTrimes} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    {term.actif ? (
                      ...
                      <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400">
                        <XCircle className="w-3.5 h-3.5" /> Clôturé
                      </span>
                    )}
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
              <h2 className="text-xl font-bold text-slate-900 dark:text-white">Nouveau Trimestre</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1">Libellé (ex: 1er Trimestre)</label>
                <input
                  type="text"
                  value={newTerm.libelle}
                  onChange={e => setNewTerm({ libelle: e.target.value })}
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex justify-end gap-3">
              ...
              <button
                ...
                disabled={saving || !newTerm.libelle}
